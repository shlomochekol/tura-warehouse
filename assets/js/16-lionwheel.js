...
/* מסירה סימון כמות-מבצע כמו "11+1"/"5+1" מתוך שם המוצר — זו הערת כמות
   מהזמנת הלקוח (איך הוא ספר את הבקבוקים), לא מוצר שונה. "רוזה 2025" ו-
   "רוזה 2025 11+1" הם אותו יין, ובלי ההסרה הזו parseItemsCell היה מחזיר
   אותם כשני "מוצרים" נפרדים במקום שורה אחת מאוחדת עם הכמות הכוללת. */
...
const CATEGORY_ALIASES=[
  /* "קברנה סובניון-ויסטה" (או וריאציות איות) מתייחס בפועל ל"קברנה ויסטה" —
     יין שונה לגמרי מ"קברנה סוביניון" — חייב להיבדק לפני הכלל הכללי
     יותר של קברנה סובניון/סוביניון למטה, אחרת היה מזוהה בטעות כקברנה
     סוביניון. */
  [/קברנה\s*סוב(י)?ניון\s*-?\s*ויסטה/,'קברנה ויסטה'],
  [/קברנה\s*סובניון/,'קברנה סוביניון'],
  [/סוביניון\s*בלאן/,'סובניון בלאן'],
  [/cabernet\s*sauvignon/i,'קברנה סוביניון'],
  [/mountain\s*peak/i,'MP'],
  [/מאונט(י)?ין(\s*פיק)?/,'MP'],           // "מאונטין"/"מאונטיין" (י' מיותרת) — שתי הכתיבים נראים בפועל
  [/\bspecial\s*edition\b/i,'ספיישל אדישן'],
  [/\blimited\s*edition\b/i,'לימיטד אדישן'],   // תופס גם "limited Edition" (אות קטנה) — case-insensitive
  [/\bsnow\b/i,'סנואו']
];
