...
function exportMapImg(){
  ...
  toast('מכין תמונה…');
  needLib('html2canvas').then(()=>html2canvas(el,{backgroundColor:'#F7F3EC',scale:2,width:el.scrollWidth,height:el.scrollHeight,windowWidth:el.scrollWidth,useCORS:true})
    .then(cv=>cv.toBlob(b=>{dl(b,'מפת-מחסן.png');toast('המפה הורדה');})))
    .catch(()=>toast('שגיאה בייצוא'));
}
function printMap(){
  const el=mapEl();if(!el){toast('המפה לא נמצאה');return;}
  /* app.css is normally loaded via <link>, not <style> — must be linked explicitly
     here too (it resolves fine, since the print iframe shares the page's base URL),
     or the grid/cell styling is missing and the map prints as a plain stacked list. */
  let css='';
